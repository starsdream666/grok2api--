...
_CONSOLE_MODEL_ALIASES: dict[str, str] = {
    "grok-4.20-fast": "grok-4.20-0309-non-reasoning",
    "grok-4.20-auto": "grok-4.20",
    "grok-4.20-expert": "grok-4.20-reasoning",
    "grok-4.20-heavy": "grok-4.20-multi-agent",
    "grok-4.20-multi-agent-0309": "grok-4.20-multi-agent",
    "grok-4.20-0309-super": "grok-4.20-0309-reasoning",
    "grok-4.20-0309-heavy": "grok-4.20-0309-reasoning",
    "grok-4.20-0309-reasoning-super": "grok-4.20-0309-reasoning",
    "grok-4.20-0309-reasoning-heavy": "grok-4.20-0309-reasoning",
    "grok-4.20-0309-non-reasoning-super": "grok-4.20-0309-non-reasoning",
    "grok-4.20-0309-non-reasoning-heavy": "grok-4.20-0309-non-reasoning",
    "grok-4.3-beta": "grok-4.3",
}


def _resolve_console_model(model: str) -> str:
    return _CONSOLE_MODEL_ALIASES.get(model, model)


def _extract_console_response_text(payload: dict[str, Any]) -> tuple[str, str | None]:
    ...
    return text, (reasoning_text or None)


async def _request_console_response(
    *,
    token: str,
    model: str,
    message: str,
    ...
    payload: dict[str, Any] = {
        "model": _resolve_console_model(model),
        "input": message,
        "stream": False,
    ...
    async with ResettableSession(**session_kwargs) as session:
        try:
            ...
            raise _transport_upstream_error(
                exc, context="Console responses transport failed"
            ) from exc
        ...
        try:
            ...
            body = response.content.decode("utf-8", "replace")[:400]
            raise UpstreamError(
                f"Console responses decode failed: {exc}",
                status=502,
                body=body,
            ) from exc
    ...
    if not isinstance(data, dict):
        raise UpstreamError(
            "Console responses returned a non-object payload",
            status=502,
            body=str(data)[:400],
        )
    return data
...
async def _console_completions(
    *,
    ...
    logger.info(
        "chat request accepted: model={} upstream={} stream={} message_count={}",
        model,
        CONSOLE_RESPONSES,
        is_stream,
        len(messages),
    ...
    if files:
        logger.warning(
            "console responses upstream currently ignores file attachments: count={}",
            len(files),
        )
    ...
    async def _run_once() -> tuple[dict[str, Any], str, str | None, int, int]:
        excluded: list[str] = []
        for attempt in range(max_retries + 1):
            ...
            try:
                try:
                    payload = await _request_console_response(
                        token=token,
                        model=model,
                        message=message,
                        temperature=temperature,
                    ...
                    full_text, reasoning_text = _extract_console_response_text(payload)
                    success = True
                    return payload, full_text, reasoning_text, attempt, selected_mode_id
        ...
        raise UpstreamError("Chat request failed without a successful response", status=502)

    if is_stream:
        async def _run_stream() -> AsyncGenerator[str, None]:
            payload, full_text, reasoning_text, attempt, _selected_mode_id = await _run_once()

            if tool_names:
                parse_result = parse_tool_calls(full_text, tool_names)
                if parse_result.calls:
                    for i, tc in enumerate(parse_result.calls):
            ...
            if emit_think and reasoning_text:
                think_chunk = make_thinking_chunk(response_id, model, reasoning_text)
                yield f"data: {orjson.dumps(think_chunk).decode()}\n\n"
            if full_text:
                text_chunk = make_stream_chunk(response_id, model, full_text)
                yield f"data: {orjson.dumps(text_chunk).decode()}\n\n"

            usage_raw = payload.get("usage") if isinstance(payload.get("usage"), dict) else {}
            prompt_tokens = int(usage_raw.get("input_tokens") or estimate_prompt_tokens(message))
            completion_tokens = int(
                usage_raw.get("output_tokens")
                or (estimate_tokens(full_text) + estimate_tokens(reasoning_text or ""))
            )
            reasoning_tokens = int(
                (usage_raw.get("output_tokens_details") or {}).get("reasoning_tokens")
                or estimate_tokens(reasoning_text or "")
            )
    ...
    if tool_names:
        ...
        if parse_result.calls:
            ...
            usage_raw = payload.get("usage") if isinstance(payload.get("usage"), dict) else {}
            completion_tokens = int(
                usage_raw.get("output_tokens") or estimate_tool_call_tokens(parse_result.calls)
            )
            return make_tool_call_response(
                ...
                usage=build_usage(pt, completion_tokens),
            )
    ...
    logger.info(
        "chat request completed: attempt={}/{} model={} upstream_model={} text_len={} reasoning_len={}",
        attempt + 1,
        max_retries + 1,
        model,
        _resolve_console_model(model),
        len(full_text),
    ...
    return make_chat_response(
        ...
        prompt_content=message,
        response_id=response_id,
        reasoning_content=reasoning_text if emit_think else None,
        usage=build_usage(
            prompt_tokens,
            completion_tokens,
            reasoning_tokens=reasoning_tokens,
